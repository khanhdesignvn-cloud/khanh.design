...
OUT=Path('/tmp/filename-privacy-evidence/live');OUT.mkdir(parents=True,exist_ok=True)
with sync_playwright() as pw:
 ...
 def privacy():
  ui=page.locator('body').inner_text()+'\n'.join(l for l in page.locator('body').aria_snapshot().splitlines() if not l.lstrip().startswith('- /url:'))
  ui+=page.locator('[alt],[title],[aria-label]').evaluate_all("es=>es.map(e=>[e.getAttribute('alt'),e.getAttribute('title'),e.getAttribute('aria-label')].join(' ')).join(' ')")
 ...
 trigger=page.locator('.file-tile').nth(1);trigger.click()
 ...
 privacy();page.screenshot(path=str(OUT/'slideshow-desktop.png'))
 ...
 page.wait_for_function("document.querySelector('.showcase-slide-stage img').naturalWidth>0")
 privacy();page.screenshot(path=str(OUT/'slideshow-mobile.png'))
 ...
 privacy();page.screenshot(path=str(OUT/'detail-mobile.png'))
