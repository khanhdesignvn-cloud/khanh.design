...
function valid(data:any){
 ...
 for(const q of data.quotes){
  ...
  if(!num(q.discountPct,0,100)||!num(q.vatPct,0,100)||!str(q.notes,2000))throw Error('Invalid quote meta');
  if(!Array.isArray(q.items)||q.items.length>200)throw Error('Invalid items');
  for(const it of q.items){
   if(!str(it.id,64)||!str(it.name,300)||!str(it.desc,1000)||!str(it.unit,20))throw Error('Invalid item');
   if(it.image!==undefined&&it.image!==''&&!IMG.test(it.image))throw Error('Invalid image');
   if(!num(it.qty,0,1e9)||!num(it.price,0,1e12))throw Error('Invalid price');
  }
 }
}
