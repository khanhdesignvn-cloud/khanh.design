...
export function useMapNavigation(enabled:boolean,memoryKey=""){
 ...
 let restored=false;
 try{const saved=JSON.parse(localStorage.getItem(memoryKey+':map')||'null');if(saved&&Number.isFinite(saved.zoom)&&Number.isFinite(saved.left)&&Number.isFinite(saved.top)){scale.current=clampZoom(saved.zoom);pending.current={left:saved.left,top:saved.top};setZoom(scale.current);restored=true;}}catch{}
 const autoFitTimer=window.setTimeout(()=>{if(restored&&pending.current){v.scrollLeft=pending.current.left;v.scrollTop=pending.current.top;pending.current=null}else if(!restored&&v.clientWidth<=640)fit()},0);
 const remember=()=>{try{localStorage.setItem(memoryKey+':map',JSON.stringify({zoom:scale.current,left:v.scrollLeft,top:v.scrollTop}))}catch{}};
 v.addEventListener('scroll',remember,{passive:true});window.addEventListener('pagehide',remember);
 ...
 return()=>{window.removeEventListener('pagehide',remember);v.removeEventListener('scroll',remember);if(autoFitTimer)window.clearTimeout(autoFitTimer);v.removeEventListener('pointerdown',down,true);v.removeEventListener('pointermove',move,true);v.removeEventListener('pointerup',up,true);v.removeEventListener('pointercancel',up,true);v.removeEventListener('wheel',wheel);v.removeEventListener('click',click,true)};
 },[enabled,memoryKey]);
